...
CREATE VIEW VieUse AS
SELECT gender, hometown, firstname, lastname
 
FROM profiles INNER JOIN users
ON profiles.user_id = users.id AND  (
...
SELECT firstname, lastname, SUM(from_user_id) OVER(PARTITION BY from_user_id) AS sum_message,
DENSE_RANK()
OVER(ORDER BY from_user_id DESC) AS 'Message_Rating'
FROM messages INNER JOIN users
ON messages.from_user_id = users.id GROUP BY from_user_id;
...
DATEDIFF(LEAD(created_at) OVER(PARTITION BY id ORDER BY id), created_at) AS 'Lag'
